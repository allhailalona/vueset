        ...
        try {
          ...
          if (!res.ok) {
            if (res.status === 401) {
              ...
              throw new Error(`Error ${res.status} in syncWithServer in store.ts: ${errorData.error || 'Unknown error'}`)
            } else if (res.status === 500) {
              const errorData = await res.json() // Try to fetch an error message
              throw new Error(`Error ${res.status} in syncWithServer in store.ts: ${errorData.error || 'Unknown error'}`) // Otherwise simply include an unknown error
            }
          }
        } catch (err) {
